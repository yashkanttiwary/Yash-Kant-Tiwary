...
import { Wifi, RefreshCw, AlertCircle, Clock, Settings, HelpCircle, Briefcase, Globe, Smartphone } from 'lucide-react';
...
export type InputType = 'actual' | 'device';
...
export const TimeCard: React.FC<TimeCardProps> = React.memo(({
  ...
  // Controls visibility of the manual offset section.
  // We automatically show it when switching to 'device' mode if it was closed, 
  // but let user toggle it too.
  const [showOffsetControls, setShowOffsetControls] = useState(true);

  // Auto-show controls when switching to device mode
  useEffect(() => {
    if (inputType === 'device') {
  ...
  const handleUseCurrentTime = () => {
    if (timeOffset !== null) {
      ...
      // Calculate time to fill based on Input Mode
      // If 'actual', fill actual time (offset 0).
      // If 'device', fill time + manualOffset.
      const offsetToUse = inputType === 'device' ? manualOffset : 0;
      const timeToFill = new Date(now.getTime() + (offsetToUse * 60000));
      
      setArrivalTime(formatTimeForInput(timeToFill));
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50 border border-slate-100 dark:border-slate-700 p-6 md:p-8 space-y-8 relative overflow-hidden">
      ...
      {/* Input Mode Toggle */}
      <div className="relative z-10 bg-slate-100 dark:bg-slate-700/50 p-1 rounded-xl flex">
        <button
      ...
      <div className="space-y-3 relative z-10">
        <div className="flex justify-between items-baseline">
          <label htmlFor="arrival-time" className="block text-sm font-semibold text-slate-700 dark:text-slate-200 uppercase tracking-wider">
            {inputType === 'actual' ? 'Arrival Time (Real World)' : 'Arrival Time (On Device)'}
          </label>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-1">
          ...
          >
            ...
            <span>Use {inputType === 'actual' ? 'Actual' : 'Device'} Time Now</span>
          </button>
      ...
      {/* Manual Clock Offset Section - ONLY VISIBLE IF DEVICE TIME IS SELECTED */}
      {inputType === 'device' && (
        <>
            <div className="h-px bg-slate-100 dark:bg-slate-700"></div>
            <div className="space-y-3 relative z-10 animate-in slide-in-from-top-4 fade-in duration-300">
                <button 
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700 space-y-4">
                    ...
                    <span>If your clock is 5 mins fast, set to "+5 min". We'll match calculations to <strong>your</strong> clock.</span>
                    </div>
